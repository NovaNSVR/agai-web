...
// Supabase Auth rejects any password found in a real breach corpus (HaveIBeenPwned
// k-anonymity check, password_hibp_enabled) with a message like "Password is known
// to be weak and easy to guess, please choose a different one." That default copy
// reads as a complexity complaint, but the real reason is that the exact password
// has appeared in a known data breach somewhere -- a strong, complex password can
// still fail this check. Detect that specific rejection and replace it with copy
// that explains the real reason and a concrete next step, rather than passing
// Supabase's generic wording straight through.
function friendlySignupError(rawMessage: string): string {
  const isBreachedPassword =
    /weak and easy to guess/i.test(rawMessage) || /known to be weak/i.test(rawMessage);

  if (isBreachedPassword) {
    return "That password has appeared in a public data breach, so we can't use it — this isn't about complexity. Try a password generated by a password manager (like 1Password, Bitwarden, or iCloud Keychain) instead.";
  }
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (!res.ok || !data.success) {
          setErrorMsg(friendlySignupError(data.error || "Something went wrong. Please try again."));
          setState("error");
          return;
        }
